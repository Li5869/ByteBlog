...
> 💡 **提示**：以下为建议截图的位置和说明。截图后替换 `images/xxx.png` 为实际图片路径即可。
...
| `images/private-message.png` | 私信聊天界面 | WebSocket 实时双向通信、心跳保活 |
| `images/admin-dashboard.png` | 管理端仪表盘（ECharts 趋势图 + 数据统计卡片） | 数据可视化、后台管理能力 |
| `images/admin-article.png` | 管理端文章管理列表（筛选/搜索/批量操作） | CRUD 管理、搜索筛选 |
| `images/swagger-api.png` | Swagger API 文档页面 | 接口规范、OpenAPI 文档自动生成 |
...
![AI Agent Skills 架构示意图](images/ai-agent-skills.png)
> *建议截图：Skills SKILL.md 文件目录 + Agent 调用 get_skill_details 工具的对话过程，展示渐进式披露机制*
...
![AI 写作工作流](images/ai-writing.gif)
> *建议截取：用户输入写作需求 → 生成计划 → 确认 → 流式输出标题/摘要/正文 → 质量评估结果的完整流程动画*
...
![AI 智能对话界面](images/ai-chat.png)
> *建议截图：对话界面展示完整的思考过程、工具调用链、以及最终回答，体现 ReAct 范式*
...
![RAG 知识库问答界面](images/knowledge-qa.png)
> *建议截图：左侧知识库文档列表 + 右侧问答交互，展示上传文档后基于文档内容提问的效果*
...
![实时通知截图](images/notification.png)
> *建议截图：收到点赞/评论通知时前端的弹窗效果，可同时展示 SSE 通知 + 通知中心列表*

![私信聊天界面](images/private-message.png)
> *建议截图：私信聊天窗口，展示 WebSocket 实时消息推送效果*
