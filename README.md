...
| 👑 **VIP 会员** | Spring 本地事务保证积分扣减/券核销/VIP 激活/订单状态原子提交，Redisson 分布式锁（事务外获取）解决确认与超时取消并发竞态，SQL 原子条件（WHERE points >= ?）防超扣 + 状态机幂等（PENDING→COMPLETED）防重复，RabbitMQ 延迟队列 + 死信兜底实现超时自动取消与最终一致性 | Spring 事务 + Redisson + RabbitMQ |
