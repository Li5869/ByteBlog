...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/XXL--Job-2.4.2-2E8B57?style=flat-square&logo=clockify&logoColor=white" alt="XXL-Job">
</p>
...
技术选型对标工业级应用：JDK 21 虚拟线程压榨并发性能、Caffeine + Redis 两级缓存对抗热点、Elasticsearch 毫秒级全文搜索、RabbitMQ 异步解耦削峰、WebSocket + SSE 双通道实时推送、LangGraph 智能 Agent 驱动 AI 写作与对话。
...
│
├── blog-common/      # 公共基础设施（WebSocket、SSE、MQ、常量、工具类）
│   ├── websocket/        WebSocket 连接管理 + 心跳 + 消息推送
│   ├── sse/              SseEmitter 通知推送
│   ├── service/          OnlineStateService（Redis 在线状态）
...
│   └── 更多定时任务可扩展
│
└── blog-application/ # 启动模块（聚合所有子模块 + Spring Boot 入口）
```
...
```
ByteBlog/
├── project-backen/           # Spring Boot 后端（11 个 Maven 子模块）
│   ├── blog-common/          # 公共基础设施
│   ├── blog-api/             # 模块间接口契约
│   ├── blog-security/        # 认证授权
│   ├── blog-article/         # 文章管理
