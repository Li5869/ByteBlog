...
ByteBlog 是一个面向开发者的 **AI 增强全栈技术博客平台**，覆盖 **文章发布、积分系统、AI 智能写作与对话、全文检索、实时互动** 等核心场景。项目采用 **模块化单体架构**，后端 14 个 Maven 模块严格分层，通过 `blog-api` 契约接口层解决跨模块循环依赖；AI 侧采用 **Spring AI + Python Agent 双引擎架构**，Spring AI 处理简单同步任务（摘要、审核），Python LangChain/LangGraph 处理复杂异步工作流（对话、写作、RAG）。
...
```
project-backen/  —— Spring Boot 4 + Maven 多模块（14 个子模块）
│
├── blog-common/      # 公共基础设施
...
```
ByteBlog/
├── project-backen/           # Spring Boot 后端（14 个 Maven 子模块）
